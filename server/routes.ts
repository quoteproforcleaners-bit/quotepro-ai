...
${contextStr}`;
  ...
  // ─── QuotePro AI Agent (3-mode: business / coach / teach) ───────────────────
...
Return exactly this JSON structure:
{
  ...
  // Shared purpose descriptions — single source of truth for both iOS and web message generation
  const SHARED_PURPOSE_DESCRIPTIONS: Record<string, string> = {
...
<body style="margin:0;padding:0;background-color:#f5f5f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  ...
  // ─── Internal Short-Link System ───

  async function createShortLink(destinationUrl: string, businessId?: string, label?: string): Promise<string> {
...
loadMonth(nextMo);
  ...
  // POST version used by Settings IntegrationCard
  app.post("/api/integrations/jobber/connect", requireAuth, requirePro as any, async (req: any, res) => {
    try {
      const clientId = process.env.JOBBER_CLIENT_ID;
      const clientSecret = process.env.JOBBER_CLIENT_SECRET;
      if (!clientId || !clientSecret) {
