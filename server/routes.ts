...
<div style="max-width:560px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
  ...
  // ─── AI Quote Assistant Routes ────────────────────────────────────────────
  {
    const {
      getAiQuoteAssistantSettings,
      upsertAiQuoteAssistantSettings,
      listConversationThreads,
      getConversationThreadById,
      listConversationMessages,
      addConversationMessage,
      updateThreadAiStatus,
      updateThreadHandoffStatus,
      getActiveIntakeSession,
      getLinqAccountByBusinessId,
      getLinqPrimaryNumber,
    } = await import("./storage");
    const { handleLinqWebhook } = await import("./services/linq/webhooks");
    const { sendLinqMessage, isLinqConfigured } = await import("./services/linq/client");
    const { generateSuggestedHumanReply } = await import("./services/linq/assistant-orchestrator");

    // Webhook — no auth, validate via signature
    app.post("/api/integrations/linq/webhook", async (req: Request, res: Response) => {
      await handleLinqWebhook(req, res);
    });

    app.get("/api/integrations/linq/health", async (req: Request, res: Response) => {
      res.json({ ok: true, configured: isLinqConfigured(), environment: process.env.LINQ_ENVIRONMENT || "sandbox" });
    });

    // Settings
    app.get("/api/ai-assistant/settings", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
    ...
    app.post("/api/ai-assistant/settings", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
    ...
    // Threads list
    app.get("/api/ai-assistant/threads", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
        const threads = await listConversationThreads(business.id, {
          aiStatus: req.query.aiStatus as string,
          handoffStatus: req.query.handoffStatus as string,
    ...
    // Thread detail
    app.get("/api/ai-assistant/threads/:id", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
        const thread = await getConversationThreadById(req.params.id, business.id);
        if (!thread) return res.status(404).json({ message: "Thread not found" });
        const messages = await listConversationMessages(thread.id, business.id, 50);
        const intake = await getActiveIntakeSession(thread.id);
        res.json({ thread, messages: messages.reverse(), intake });
      } catch (e: any) { res.status(500).json({ message: e.message }); }
    });

    // Pause / resume / take-over / release
    app.post("/api/ai-assistant/threads/:id/pause", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
        await updateThreadAiStatus(req.params.id, business.id, "paused");
        res.json({ ok: true });
      } catch (e: any) { res.status(500).json({ message: e.message }); }
    });

    app.post("/api/ai-assistant/threads/:id/resume", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
        await updateThreadAiStatus(req.params.id, business.id, "active");
        await updateThreadHandoffStatus(req.params.id, business.id, "ai");
        res.json({ ok: true });
      } catch (e: any) { res.status(500).json({ message: e.message }); }
    });

    app.post("/api/ai-assistant/threads/:id/take-over", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
        await updateThreadAiStatus(req.params.id, business.id, "paused");
        await updateThreadHandoffStatus(req.params.id, business.id, "human");
        res.json({ ok: true });
      } catch (e: any) { res.status(500).json({ message: e.message }); }
    });

    app.post("/api/ai-assistant/threads/:id/release-to-ai", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
        await updateThreadAiStatus(req.params.id, business.id, "active");
    ...
    // Human reply
    app.post("/api/ai-assistant/threads/:id/reply", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
        const thread = await getConversationThreadById(req.params.id, business.id);
        ...
        await sendLinqMessage({ from: primaryNum?.phoneNumber || "", to: thread.phoneNumber, body });
        await addConversationMessage({ threadId: thread.id, direction: "outbound", provider: "linq", sender: primaryNum?.phoneNumber, recipient: thread.phoneNumber, body });
        res.json({ ok: true });
      } catch (e: any) { res.status(500).json({ message: e.message }); }
    });

    // Suggested reply
    app.post("/api/ai-assistant/threads/:id/generate-suggested-reply", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
        const thread = await getConversationThreadById(req.params.id, business.id);
        if (!thread) return res.status(404).json({ message: "Thread not found" });
        const messages = await listConversationMessages(thread.id, business.id, 10);
        const transcript = messages.reverse().map((m) => ({ role: (m.direction === "inbound" ? "user" : "assistant") as "user" | "assistant", content: m.body }));
        const intake = await getActiveIntakeSession(thread.id);
        const reply = await generateSuggestedHumanReply(transcript, business.companyName, { customerName: thread.customerName ?? undefined, intake: intake as any });
        res.json({ suggestedReply: reply });
      } catch (e: any) { res.status(500).json({ message: e.message }); }
    });

    // Test message (sandbox)
    app.post("/api/ai-assistant/test/send", requireAuth, requirePro as any, async (req: any, res: Response) => {
      try {
        const business = await getBusinessByOwner(req.session.userId!);
        if (!business) return res.status(404).json({ message: "Business not found" });
        ...
        await handleHook(fakeReq, fakeRes);
        res.json({ ok: true, simulated: { from, to, body } });
      } catch (e: any) { res.status(500).json({ message: e.message }); }
    });
  }
