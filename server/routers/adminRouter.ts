...
const router = Router();
  ...
  router.get("/deleted-records", requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  router.post("/restore/:type/:id", requireAuth, async (req: Request, res: Response) => {
    try {
  ...
  router.post("/billing-recovery", async (req: Request, res: Response) => {
    const adminKey = req.headers["x-admin-key"] as string;
    const ADMIN_KEY = process.env.ADMIN_API_KEY || process.env.ADMIN_GRANT_PRO_SECRET || "";
    if (!adminKey || adminKey !== ADMIN_KEY) {
      return res.status(401).json({ message: "Unauthorized" });
...
// ─── Field Admin (merged from fieldAdminRouter) ───────────────────────────────
...
async function getFieldBusinessId(userId: string): Promise<string | null> {
  const [biz] = await db
    .select()
    .from(businesses)
...
router.get("/employees", requireAuth, async (req: Request, res: Response) => {
  try {
    const businessId = await getFieldBusinessId(req.session.userId!);
    if (!businessId) return res.status(404).json({ message: "Business not found" });
    const emps = await db.select().from(employees).where(eq(employees.businessId, businessId)).orderBy(asc(employees.name));
    const today = new Date().toISOString().slice(0, 10);
    const result = await Promise.all(emps.map(async (e) => {
      const todayAssignments = await db.select().from(jobAssignments).where(and(eq(jobAssignments.employeeId, e.id), eq(jobAssignments.assignedDate, today)));
      return { ...e, pin: undefined, todayJobCount: todayAssignments.length, todayStatuses: todayAssignments.map((a) => a.status) };
    }));
...
router.post("/employees", requireAuth, async (req: Request, res: Response) => {
  try {
    const businessId = await getFieldBusinessId(req.session.userId!);
    if (!businessId) return res.status(404).json({ message: "Business not found" });
    const { name, email, phone, pin, role } = req.body as { name?: string; email?: string; phone?: string; pin?: string; role?: string };
    if (!name || !email || !pin) return res.status(400).json({ message: "Name, email, and PIN are required" });
    ...
    const [created] = await db.insert(employees).values({ businessId, name: name.trim(), email: email.toLowerCase().trim(), phone: phone?.trim() ?? "", role: role?.trim() ?? "Cleaner", pin: hashedPin, isActive: true, color, status: "active" }).returning();
    const origin = (req.headers.origin as string) || `${req.protocol}://${req.get("host")}`;
    sendEmployeeWelcomeEmail({ businessId, employeeName: name.trim(), employeeEmail: email.toLowerCase().trim(), pin: plainPin, portalUrl: `${origin}/employee/login` }).catch((e) => console.error("[employee invite email]", e));
    return res.json({ ...created, pin: undefined });
  } catch (err: any) {
    console.error("[admin/employees POST]", err);
    if (err?.code === "23505") return res.status(409).json({ message: "An employee with that email already exists" });
    return res.status(500).json({ message: "Failed to create employee" });
  }
});

router.post("/employees/:id/invite", requireAuth, async (req: Request, res: Response) => {
  try {
    const businessId = await getFieldBusinessId(req.session.userId!);
    if (!businessId) return res.status(404).json({ message: "Business not found" });
    const { id } = req.params;
    const [emp] = await db.select().from(employees).where(and(eq(employees.id, id), eq(employees.businessId, businessId))).limit(1);
    if (!emp) return res.status(404).json({ message: "Employee not found" });
    const origin = (req.headers.origin as string) || `${req.protocol}://${req.get("host")}`;
    await sendEmployeePortalReminderEmail({ businessId, employeeName: emp.name, employeeEmail: emp.email, portalUrl: `${origin}/employee/login` });
...
router.patch("/employees/:id", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (!businessId) return res.status(404).json({ message: "Business not found" });
    const { id } = req.params;
    const { name, phone, role, isActive, pin } = req.body as { name?: string; phone?: string; role?: string; isActive?: boolean; pin?: string };
    ...
    if (isActive !== undefined) { updates.isActive = isActive; updates.status = isActive ? "active" : "inactive"; }
    if (pin) {
      if (!/^\d{4,6}$/.test(pin)) return res.status(400).json({ message: "PIN must be 4–6 digits" });
      updates.pin = await bcrypt.hash(pin, 12);
    }
    const [updated] = await db.update(employees).set(updates).where(and(eq(employees.id, id), eq(employees.businessId, businessId))).returning();
...
router.delete("/employees/:id", requireAuth, async (req: Request, res: Response) => {
  try {
    const businessId = await getFieldBusinessId(req.session.userId!);
    if (!businessId) return res.status(404).json({ message: "Business not found" });
    await db.update(employees).set({ isActive: false, status: "inactive", updatedAt: new Date() }).where(and(eq(employees.id, req.params.id), eq(employees.businessId, businessId)));
...
router.post("/jobs/:jobId/assign", requireAuth, async (req: Request, res: Response) => {
  try {
    const businessId = await getFieldBusinessId(req.session.userId!);
    if (!businessId) return res.status(404).json({ message: "Business not found" });
    const { jobId } = req.params;
    const { employeeId, assignedDate } = req.body as { employeeId?: string; assignedDate?: string };
    if (!employeeId || !assignedDate) return res.status(400).json({ message: "employeeId and assignedDate required" });
    ...
    if (!emp) return res.status(404).json({ message: "Employee not found" });
    const [existing] = await db.select().from(jobAssignments).where(and(eq(jobAssignments.jobId, jobId), eq(jobAssignments.employeeId, employeeId))).limit(1);
    if (existing) return res.json(existing);
    const [assignment] = await db.insert(jobAssignments).values({ jobId, employeeId, businessId, assignedDate, status: "assigned" }).returning();
...
router.delete("/jobs/:jobId/assignments/:assignmentId", requireAuth, async (req: Request, res: Response) => {
  try {
    const businessId = await getFieldBusinessId(req.session.userId!);
    if (!businessId) return res.status(404).json({ message: "Business not found" });
    await db.delete(jobAssignments).where(and(eq(jobAssignments.id, req.params.assignmentId), eq(jobAssignments.businessId, businessId)));
    return res.json({ success: true });
  } catch (err) {
...
router.get("/jobs/:jobId/assignments", requireAuth, async (req: Request, res: Response) => {
  try {
    const businessId = await getFieldBusinessId(req.session.userId!);
    if (!businessId) return res.status(404).json({ message: "Business not found" });
    const assignments = await db.select().from(jobAssignments).where(and(eq(jobAssignments.jobId, req.params.jobId), eq(jobAssignments.businessId, businessId))).orderBy(asc(jobAssignments.createdAt));
...
router.get("/dashboard/field-status", requireAuth, async (req: Request, res: Response) => {
  try {
    const businessId = await getFieldBusinessId(req.session.userId!);
    if (!businessId) return res.status(404).json({ message: "Business not found" });
    const today = new Date().toISOString().slice(0, 10);
    const emps = await db.select().from(employees).where(and(eq(employees.businessId, businessId), eq(employees.isActive, true))).orderBy(asc(employees.name));
    const result = await Promise.all(emps.map(async (emp) => {
      const assignments = await db.select().from(jobAssignments).where(and(eq(jobAssignments.employeeId, emp.id), eq(jobAssignments.assignedDate, today))).orderBy(asc(jobAssignments.createdAt));
      const enriched = await Promise.all(assignments.map(async (a) => {
        const [job] = await db.select().from(jobs).where(eq(jobs.id, a.jobId)).limit(1);
        let customer = null;
        if (job?.customerId) { [customer] = await db.select().from(customers).where(eq(customers.id, job.customerId)).limit(1); }
        return { ...a, job: job ? { id: job.id, address: job.address, startDatetime: job.startDatetime, estimatedDurationMinutes: job.estimatedDurationMinutes } : null, customerName: customer ? `${customer.firstName} ${customer.lastName}`.trim() : "Customer" };
      }));
      return { employee: { id: emp.id, name: emp.name, color: emp.color, role: emp.role, phone: emp.phone }, assignments: enriched };
    }));
...
router.get("/events/log", requireAuth, async (req: Request, res: Response) => {
  try {
    const businessId = await getFieldBusinessId(req.session.userId!);
    if (!businessId) return res.status(404).json({ message: "Business not found" });
    const rows = await db
      .select({ assignmentId: jobAssignments.id, status: jobAssignments.status, checkinTime: jobAssignments.checkinTime, checkoutTime: jobAssignments.checkoutTime, durationMinutes: jobAssignments.durationMinutes, employeeName: employees.name, employeeColor: employees.color, customerFirstName: customers.firstName, customerLastName: customers.lastName, jobAddress: jobs.address })
      .from(jobAssignments)
      .innerJoin(employees, eq(jobAssignments.employeeId, employees.id))
      .innerJoin(jobs, eq(jobAssignments.jobId, jobs.id))
      .innerJoin(customers, eq(jobs.customerId, customers.id))
      .where(and(eq(employees.businessId, businessId), sql`${jobAssignments.status} IN ('checked_in', 'completed')`))
      .orderBy(desc(jobAssignments.checkinTime))
      .limit(20);
    const events = rows.flatMap(row => {
      const items: any[] = [];
      if (row.checkinTime) items.push({ type: "checkin", assignmentId: row.assignmentId, employeeName: row.employeeName, employeeColor: row.employeeColor, customerName: `${row.customerFirstName} ${row.customerLastName}`.trim(), address: row.jobAddress, timestamp: row.checkinTime, durationMinutes: null });
      if (row.checkoutTime) items.push({ type: "checkout", assignmentId: row.assignmentId, employeeName: row.employeeName, employeeColor: row.employeeColor, customerName: `${row.customerFirstName} ${row.customerLastName}`.trim(), address: row.jobAddress, timestamp: row.checkoutTime, durationMinutes: row.durationMinutes });
      return items;
    });
...
router.get("/events/stream", requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    req.on("close", () => { clearInterval(heartbeat); sseClients.get(businessId)?.delete(res); });
  } catch (err) {
