...
.icon{font-size:48px;margin-bottom:16px;}h2{margin:0 0 8px;color:#333;}p{color:#666;margin:0;}</style>
  ...
  router.get("/api/integrations/jobber/connect", requireAuth, requireGrowth, async (req: any, res) => {
    try {
  ...
  router.post("/api/integrations/jobber/connect", requireAuth, requireGrowth, async (req: any, res) => {
    try {
      const clientId = process.env.JOBBER_CLIENT_ID;
      const clientSecret = process.env.JOBBER_CLIENT_SECRET;
      if (!clientId || !clientSecret) {
...
<script>if(window.opener){window.opener.postMessage('jobber_connected','*');}setTimeout(()=>{window.close();},1500);</script>
  ...
  router.post("/api/integrations/jobber/disconnect", requireAuth, requireGrowth, async (req: any, res) => {
    try {
  ...
  router.put("/api/integrations/jobber/settings", requireAuth, requireGrowth, async (req: any, res) => {
    try {
  ...
  router.post("/api/integrations/jobber/sync-quote/:quoteId", requireAuth, requireGrowth, async (req: any, res) => {
    try {
  ...
  router.get("/api/integrations/jobber/clients", requireAuth, requireGrowth, async (req: any, res) => {
    try {
      const client = new JobberClient(req.session.userId);
      const conn = await client.loadConnection();
      if (!conn) return res.status(404).json({ error: "No Jobber connection found" });
  ...
  router.post("/api/integrations/jobber/import-clients", requireAuth, requireGrowth, async (req: any, res) => {
    try {
  ...
  router.post("/api/integrations/jobber/save-token", requireAuth, requireGrowth, async (req: any, res: Response) => {
    try {
      const business = await getBusinessByOwner(req.session.userId!);
      if (!business) return res.status(404).json({ message: "Business not found" });
  ...
  router.post("/api/integrations/jobber/sync-quote-token/:quoteId", requireAuth, requireGrowth, async (req: any, res: Response) => {
    try {
      const business = await getBusinessByOwner(req.session.userId!);
      if (!business) return res.status(404).json({ message: "Business not found" });
      const token = (business as any).jobberApiToken;
