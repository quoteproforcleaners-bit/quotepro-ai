...
// Pricing tiers catalog (Solo / Growth / Pro / Scale)
export const pricingTiers = pgTable("pricing_tiers", {
  id: serial("id").primaryKey(),
  name: varchar("name", { length: 32 }).notNull().unique(),
  displayName: varchar("display_name", { length: 64 }).notNull(),
  monthlyPrice: integer("monthly_price").notNull(), // cents
  annualPrice: integer("annual_price").notNull(), // cents (per month, billed annually)
  quoteLimit: integer("quote_limit").notNull(),
...
// Per-user subscription records (one active row per user). Adds usage + overage
// + billing-period tracking and a legacy_locked_price for grandfathered users.
export const subscriptions = pgTable("subscriptions", {
  ...
  interval: varchar("interval", { length: 16 }).notNull().default("monthly"), // 'monthly' | 'annual'
  quotesUsedCurrentPeriod: integer("quotes_used_current_period").notNull().default(0),
  overageChargesCurrentPeriod: integer("overage_charges_current_period").notNull().default(0), // cents
  billingPeriodStart: timestamp("billing_period_start"),
  billingPeriodEnd: timestamp("billing_period_end"),
  legacyPlanName: varchar("legacy_plan_name", { length: 32 }),
  legacyLockedPrice: integer("legacy_locked_price"), // cents/month, locked for legacy users
  legacyLockedUntil: timestamp("legacy_locked_until"),
  stripeSubscriptionId: text("stripe_subscription_id"),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
}, (table) => ({
  userIdx: index("subscriptions_user_id_idx").on(table.userId),
  tierIdx: index("subscriptions_pricing_tier_id_idx").on(table.pricingTierId),
}));
export type Subscription = typeof subscriptions.$inferSelect;
export type InsertSubscription = typeof subscriptions.$inferInsert;

// Per-quote usage events for monthly counting and overage billing.
export const quoteUsage = pgTable("quote_usage", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id")
    .notNull()
    .references(() => users.id, { onDelete: "cascade" }),
  ...
  userPeriodIdx: index("quote_usage_user_period_idx").on(table.userId, table.billingPeriod),
  quoteIdx: index("quote_usage_quote_id_idx").on(table.quoteId),
}));
export type QuoteUsage = typeof quoteUsage.$inferSelect;
export type InsertQuoteUsage = typeof quoteUsage.$inferInsert;
