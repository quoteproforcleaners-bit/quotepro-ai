...
function LineItemRow({ item }: { item: LineItem & { label: string; amount: number } }) {
 ...
 const color = isNeg ?"text-emerald-600": item.type ==="base"?"text-slate-700":"text-slate-600";
 return (
...
function WarningChip({ w }: { w: Warning & { type?: string } }) {
 ...
 isInfo ?"text-blue-600":"text-amber-700"
 }`}
 >
 {config.action.label} →
 </a>
...
function ManualAdjustmentPanel({ adjustment, onChange }: { adjustment: ManualAdjustment; onChange: (a: ManualAdjustment) => void }) {
 ...
 className={`w-9 h-9 flex items-center justify-center text-sm font-bold transition-colors ${sign === s ?"bg-primary-600 text-white":"bg-white text-slate-500 hover:bg-slate-50"}`}>
 ...
 className="w-full pl-6 pr-3 py-2 text-sm border border-slate-200 rounded-lg bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary-500/30"
 />
 </div>
 </div>
 ...
 className="w-full px-3 py-2 text-xs border border-slate-200 rounded-lg bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-primary-500/30 placeholder-slate-400"
 />
 {rawValue && parseFloat(rawValue) > 0 && (
 <div className={`text-xs font-medium rounded-lg px-3 py-2 flex items-center gap-1.5 ${adjustment.amount > 0 ?"bg-amber-50 text-amber-700":"bg-emerald-50 text-emerald-700"}`}>
...
function Section({ title, children, defaultOpen = true }: { title: string; children: React.ReactNode; defaultOpen?: boolean }) {
 ...
 className="w-full flex items-center justify-between text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 hover:text-slate-700 transition-colors">
 {title}
 <ChevronDown className={`w-3.5 h-3.5 transition-transform ${open ?"rotate-180":""}`} />
 </button>
...
export function ResidentialLivePreview({ result, selectedTier, priceOverride, adjustment, onAdjustmentChange, frequency, beds }: ResidentialPreviewProps) {
 ...
 <div className="text-center py-5 px-4 bg-gradient-to-b from-slate-50 to-white rounded-2xl border border-slate-200/80">
 <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">
 {selectedTier.charAt(0).toUpperCase() + selectedTier.slice(1)} · {tier.name}
 </p>
 <AnimatedPrice value={total} className="text-4xl font-black tracking-tight text-slate-900"/>
 {isRecurring && tier.firstCleanPrice ? (
 <div className="mt-2 space-y-0.5">
 <p className="text-xs text-slate-400">per visit (recurring)</p>
 <p className="text-xs font-medium text-amber-600">First visit: ${tier.firstCleanPrice.toFixed(0)}</p>
 </div>
 ) : (
 <p className="mt-1 text-xs text-slate-400">{isRecurring ?"per visit":"one-time"}</p>
 )}
 <div className="inline-flex items-center gap-1.5 mt-3 px-3 py-1 bg-slate-100 rounded-full">
 <Clock className="w-3 h-3 text-slate-400"/>
 <span className="text-xs text-slate-500 font-medium">~{tier.totalHours}h labor estimate</span>
 ...
 <span className="text-xs text-slate-500 leading-tight italic">{adjustment.note ||"Manual adjustment"}</span>
 ...
 return (
 <div key={t} className={`flex items-center justify-between py-1.5 px-2 rounded-lg ${isActive ?"bg-primary-50":""}`}>
 <span className={`text-xs capitalize ${isActive ?"font-bold text-primary-700":"text-slate-500"}`}>{t} · {d.name}</span>
 <span className={`text-xs font-bold tabular-nums ${isActive ?"text-primary-700":"text-slate-600"}`}>${d.price.toLocaleString()}</span>
 </div>
 );
 })}
 </Section>
...
export function CommercialLivePreview({ result, facilityName, facilityType, totalSqFt, adjustment, onAdjustmentChange }: CommercialPreviewProps) {
 ...
 {/* Primary total */}
 <div className="text-center py-5 px-4 bg-gradient-to-b from-slate-50 to-white rounded-2xl border border-slate-200/80">
 {facilityName && <p className="text-xs text-slate-400 mb-1 truncate">{facilityName}</p>}
 <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2">Per Visit (Enhanced)</p>
 <AnimatedPrice value={adjPerVisit} className="text-4xl font-black tracking-tight text-slate-900"/>
 <div className="flex items-center justify-center gap-1 mt-1">
 <p className="text-xs text-slate-400">per cleaning visit</p>
 <Tooltip
 ...
 <AnimatedPrice value={value} className="text-sm font-bold text-slate-800"/>
 </div>
 ))}
 </div>
 ...
 item.type ==="surcharge"?"text-amber-700":"text-slate-700"
 }`}>
 ...
 />
 <span className="text-sm font-black text-slate-900 tabular-nums">${Math.round(adjPerVisit).toLocaleString()}</span>
 </div>
 ...
 <LabelWithTooltip label={label} tooltip={tip} labelClass="text-xs text-slate-500"/>
...
export function LivePreviewPanel({ title ="Live Quote Preview", children, isEmpty }: PreviewPanelProps) {
 return (
 <div
 className="sticky top-6 w-80 shrink-0 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden"
 style={{ maxHeight:"calc(100vh - 48px)", display:"flex", flexDirection:"column"}}
 >
 <div className="flex items-center gap-2 px-5 py-3.5 border-b border-slate-100 shrink-0 bg-gradient-to-r from-primary-600 to-violet-600">
 ...
 <ChevronRight className="w-8 h-8 text-slate-200 mb-3"/>
 <p className="text-sm font-medium text-slate-400">Fill in property details</p>
 <p className="text-xs text-slate-300 mt-1">Your live price will appear here</p>
 </div>
 ) : children}
 </div>
 </div>
