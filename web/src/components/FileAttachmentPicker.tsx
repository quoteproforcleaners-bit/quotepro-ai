    ...
    <div>
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {selectedFiles.map(f => (
          <span
            key={f.id}
            className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-300 border border-primary-200 dark:border-primary-700"
          >
            <File className="w-3 h-3" />
            {f.originalName.length > 20 ? f.originalName.slice(0, 20) + "…" : f.originalName}
            <button onClick={() => remove(f.id)} className="ml-0.5 hover:text-primary-900 dark:hover:text-white">
              <X className="w-3 h-3" />
            </button>
          </span>
        ))}
      ...
      {open && (
        <div className="mt-2 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden bg-white dark:bg-slate-800">
          {isLoading ? (
            <p className="text-sm text-slate-400 p-4">Loading files...</p>
          ) : files.length === 0 ? (
            <div className="p-4 text-center">
              <FolderOpen className="w-6 h-6 text-slate-300 dark:text-slate-600 mx-auto mb-1" />
              <p className="text-sm text-slate-500 dark:text-slate-400">No files in library.</p>
              <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">Upload files in the File Library first.</p>
            </div>
          ) : (
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors ${
                      isSelected ? "bg-primary-50 dark:bg-primary-900/20" : "hover:bg-slate-50 dark:hover:bg-slate-700"
                    }`}
                  >
                    <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 transition-colors ${
                      isSelected ? "bg-primary-600 border-primary-600" : "border-slate-300 dark:border-slate-600"
                    }`}>
                      {isSelected && <Check className="w-2.5 h-2.5 text-white" />}
                    </div>
                    <FileIcon className="w-4 h-4 text-slate-400 flex-shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-slate-700 dark:text-slate-200 truncate">{f.originalName}</p>
