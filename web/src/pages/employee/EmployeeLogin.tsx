...
export default function EmployeeLogin() {
  ...
  const handlePinKey = async (key: string) => {
    ...
    if (next.length === 6) {
      ...
      setError(null);
      try {
        const result = await loginEmployee(email.trim(), next);
    ...
    <div style={styles.page}>
      ...
      {step === "email" ? (
        ...
        <div style={styles.pinSection}>
          <button onClick={() => { setStep("email"); setPin(""); setError(null); }} style={styles.backBtn}>
            ← {email}
          </button>

          <h2 style={styles.heading}>Enter your PIN</h2>
          <p style={styles.subtext}>Enter your 6-digit PIN</p>
          ...
          {loading && <p style={{ textAlign: "center", color: "#0F6E56", margin: "8px 0" }}>Signing in...</p>}

          {/* PIN Pad */}
          <div style={styles.pinGrid}>
            {PIN_PAD.map(([num, letters], idx) => {
              if (num === "") {
                return <div key={idx} style={styles.pinEmpty} />;
              }
              return (
                <button
                  key={idx}
