...
function getCategoryColor(category: string): string {
  const map: Record<string, string> = {
    contracts: "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300",
    proposals: "bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300",
    checklists: "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300",
    invoices: "bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300",
    branding: "bg-pink-100 text-pink-700 dark:bg-pink-900/30 dark:text-pink-300",
    training: "bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300",
    templates: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-300",
    general: "bg-slate-100 text-slate-600 dark:bg-slate-700/50 dark:text-slate-300",
  };
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <div
        ...
        onClick={() => fileInputRef.current?.click()}
        className="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-8 text-center cursor-pointer hover:border-primary-400 hover:bg-primary-50/50 dark:hover:bg-primary-900/10 transition-all group"
      >
        ...
        <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">PDF, Word, Excel, images, and more</p>
      </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <div className="py-12 text-center">
            <FolderOpen className="mx-auto mb-3 text-slate-300 dark:text-slate-600" size={40} />
            <p className="text-slate-500 dark:text-slate-400 font-medium">No files yet</p>
                    ...
                    {isEditing ? (
                      <div className="mt-3 space-y-2">
                        <select
                          ...
                          className="w-full px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100"
                        >
                          {CATEGORIES.map(c => (
                        ...
                        <div className="flex gap-2">
                          <input
                            ...
                            className="flex-1 px-3 py-2 text-sm border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 placeholder-slate-400"
                          />
                          <button
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{file.description}</p>
                    ) : null}
                  </div>
                </div>
              </Card>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-md">
            <div className="flex items-center justify-between p-5 border-b border-slate-200 dark:border-slate-700">
              <h3 className="font-semibold text-slate-800 dark:text-slate-100">Upload Files</h3>
              <button
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100"
                >
                  {CATEGORIES.map(c => (
                    <option key={c.value} value={c.value}>{c.label}</option>
                  ))}
