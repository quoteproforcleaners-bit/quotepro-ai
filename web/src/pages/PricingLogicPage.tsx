...
const SOURCE_BADGES: Record<string, { label: string; color: string }> = {
  inferred: { label: "Inferred from your data", color: "text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20" },
  "ai-recommended": { label: "AI Recommended", color: "text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/20" },
  user: { label: "User-defined", color: "text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800" },
};
...
function JobsStep({ onNext }: { onNext: () => void }) {
  ...
  const inp = "w-full border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500";
  const lbl = "text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Step 1 — Import Your Past Jobs</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Upload or manually enter 3–20 past jobs. The more you add, the better the analysis.</p>
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => fileRef.current?.click()}
            className="flex items-center gap-1.5 px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800"
          >
      ...
      <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg flex items-start gap-2 text-sm text-blue-700 dark:text-blue-300">
        ...
        <span><strong>CSV format:</strong> Include columns like <code className="bg-blue-100 dark:bg-blue-900/40 px-1 rounded">customer_name, service_type, sqft, beds, baths, condition, pets, frequency, final_price, won, notes</code>. Only <strong>final_price</strong> is required.</span>
      </div>
      ...
      {showForm && (
        <Card>
          <div className="p-5">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-4">Add Past Job</h3>
            ...
            <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-slate-100 dark:border-slate-700">
              <button onClick={() => { setShowForm(false); setForm(emptyJob()); }} className="px-4 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg">Cancel</button>
      ...
      {isLoading ? (
        ...
        <Card>
          <div className="py-16 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm">No jobs yet. Add your first past job above or import a CSV.</p>
          </div>
        </Card>
      ) : (
        <Card>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                {jobs.map(j => (
                  <tr key={j.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    <td className="px-4 py-2.5 font-medium text-slate-900 dark:text-white">{j.customerName || "—"}</td>
                    <td className="px-4 py-2.5 text-slate-600 dark:text-slate-300 capitalize">{j.serviceType?.replace("-", " ")}</td>
                    <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400">{j.sqft ? `${j.sqft.toLocaleString()}` : "—"}</td>
                    <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400">{j.beds || "—"} / {j.baths || "—"}</td>
                    <td className="px-4 py-2.5">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${j.conditionLevel === "heavy" ? "bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400" : j.conditionLevel === "light" ? "bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400" : "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300"}`}>
                        {j.conditionLevel}
                      </span>
                    </td>
                    <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400 capitalize">{j.frequency}</td>
                    <td className="px-4 py-2.5">{j.pets ? <span className="text-amber-600 dark:text-amber-400 text-xs">Yes</span> : <span className="text-slate-400 text-xs">No</span>}</td>
                    <td className="px-4 py-2.5 font-semibold text-slate-900 dark:text-white">${j.finalPrice.toFixed(0)}</td>
...
function QuestionnaireStep({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
    ...
    <div>
      ...
      <div className="relative">
        ...
        <input
          ...
          step={opts?.step ?? 1}
          min={opts?.min ?? 0}
          className={`w-full border border-slate-200 dark:border-slate-700 rounded-lg py-2 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 ${opts?.prefix ? "pl-8 pr-3" : opts?.suffix ? "pl-3 pr-8" : "px-3"}`}
  ...
  const tog = (field: string, label: string, hint?: string) => (
    <div className="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800">
      ...
      <button
        ...
        className={`text-2xl transition-colors ${(form as any)[field] ? "text-primary-600" : "text-slate-300 dark:text-slate-600"}`}
      >
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Step 2 — Your Pricing Assumptions</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Tell us how you think about pricing. We'll use this alongside your past jobs to build your rules.</p>
      </div>

      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          <div className="p-5 space-y-4">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Baseline Pricing</h3>
        ...
        <Card>
          <div className="p-5 space-y-4">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Service Multipliers</h3>
            <div className="grid grid-cols-2 gap-4">
        ...
        <Card>
          <div className="p-5">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-1">Discounts</h3>
            <div className="grid grid-cols-2 gap-4">
        ...
        <Card>
          <div className="p-5">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white mb-2">Pricing Factors</h3>
      ...
      <Card>
        <div className="p-5">
          <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block">Additional Notes (optional)</label>
          <textarea
            className="w-full border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 h-20 resize-none"
      ...
      <div className="flex items-center justify-between pt-2">
        <button onClick={onBack} className="flex items-center gap-2 px-4 py-2.5 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl text-sm">
          Back
        </button>
        <div className="flex gap-3">
          <button onClick={() => handleSave(false)} disabled={saving} className="px-4 py-2.5 border border-slate-200 dark:border-slate-700 rounded-xl text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">
            Save
          </button>
...
function AnalysisStep({ onNext, onBack }: { onNext: () => void; onBack: () => void }) {
  ...
  const inp = "w-full border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-1.5 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500";

  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Step 3 — Analysis & Pricing Rules</h2>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Review your inferred pricing patterns, revenue opportunities, and editable pricing rules.</p>
        </div>
        <button
          onClick={runAnalysis}
          disabled={analyzing || jobs.length < 3}
          className="flex items-center gap-2 px-4 py-2 border border-primary-200 dark:border-primary-800 text-primary-700 dark:text-primary-300 rounded-lg text-sm hover:bg-primary-50 dark:hover:bg-primary-900/20 disabled:opacity-50"
      ...
      {!analysisLoading && !analysis && (
        <Card>
          <div className="py-16 text-center space-y-4">
            ...
            <div>
              <p className="text-lg font-semibold text-slate-900 dark:text-white">Ready to analyze your pricing</p>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">We'll analyze your {jobs.length} jobs and questionnaire to find patterns and build your pricing rules.</p>
            </div>
            <button
              onClick={runAnalysis}
              disabled={analyzing || jobs.length < 3}
            ...
            {jobs.length < 3 && <p className="text-xs text-amber-600 dark:text-amber-400">Add at least 3 past jobs first (you have {jobs.length}).</p>}
          </div>
        </Card>
      )}
        ...
        <>
          ...
          <div className="flex gap-1 border-b border-slate-200 dark:border-slate-700">
            {[
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${activeTab === t.key ? "border-primary-600 text-primary-600 dark:text-primary-400" : "border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300"}`}
              >
          ...
          {activeTab === "summary" && (
            <div className="space-y-4">
              ...
              {inf.pricingStyle && (
                <Card>
                  <div className="p-4">
                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-2">Pricing Style</p>
                    <p className="text-sm text-slate-700 dark:text-slate-300">{inf.pricingStyle}</p>
                  </div>
                </Card>
              )}
              {inf.observations?.length > 0 && (
                <Card>
                  <div className="p-4">
                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-3">Key Observations</p>
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500">Analysis ran on {new Date(analysis.createdAt).toLocaleDateString()} using {analysis.jobCount} jobs.</p>
            </div>
          )}
          ...
          {activeTab === "opportunities" && (
            <div className="space-y-4">
              {opportunities.length === 0 ? (
                <Card><div className="py-10 text-center text-slate-400 dark:text-slate-500 text-sm">No revenue opportunities identified from current data.</div></Card>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <h4 className="text-sm font-semibold text-slate-900 dark:text-white">{op.title}</h4>
                          <span className={`text-xs px-2 py-0.5 rounded-full ${op.confidence === "high" ? "bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400" : op.confidence === "medium" ? "bg-yellow-50 dark:bg-yellow-900/20 text-yellow-600 dark:text-yellow-400" : "bg-slate-100 dark:bg-slate-700 text-slate-500"}`}>
                            {op.confidence} confidence
                          </span>
                        </div>
                        <p className="text-sm text-slate-600 dark:text-slate-300 mb-2">{op.description}</p>
                        {op.dataPoints && <p className="text-xs text-slate-400 dark:text-slate-500 italic">{op.dataPoints}</p>}
                      </div>
                      {op.estimatedImpact && (
                        <div className="text-right shrink-0">
                          <p className="text-xs text-slate-400 dark:text-slate-500">Est. Impact</p>
                          <p className="text-sm font-bold text-green-600 dark:text-green-400">{op.estimatedImpact}</p>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {activeTab === "rules" && (
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                <p className="text-sm text-slate-500 dark:text-slate-400">{activeRules.length} of {rules.length} rules active</p>
                <button
                  onClick={publish}
              ...
              {rulesLoading ? <div className="flex items-center justify-center py-8"><Spinner /></div> : rules.length === 0 ? (
                <Card><div className="py-10 text-center text-slate-400 dark:text-slate-500 text-sm">No rules yet. Run the analysis to generate rules.</div></Card>
              ) : rules.map(rule => {
                    ...
                    <div className="p-4">
                      <div className="flex items-center gap-3">
                        <button
                          ...
                          className={`shrink-0 text-xl transition-colors ${rule.active ? "text-primary-600" : "text-slate-300 dark:text-slate-600"}`}
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          {isEditing ? (
                            <input className="w-full border border-primary-300 dark:border-primary-600 rounded-lg px-2 py-1 text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-none"
                              value={editForm.label} onChange={e => setEditForm((f: any) => ({...f, label: e.target.value}))} />
                          ) : (
                            <div className="flex items-center gap-2 flex-wrap">
                              <span className={`font-medium text-sm ${rule.active ? "text-slate-900 dark:text-white" : "text-slate-400 dark:text-slate-500"}`}>{rule.label}</span>
                              <span className="text-xs text-slate-400 dark:text-slate-500">{RULE_TYPE_LABELS[rule.ruleType] || rule.ruleType}</span>
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <button onClick={() => setExpandedId(isExpanded ? null : rule.id)} className="p-1.5 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700">
                            {isExpanded ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
                          </button>
                          {!isEditing && (
                            <button onClick={() => startEdit(rule)} className="p-1.5 text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700">
                              <Edit2 className="w-3.5 h-3.5" />
                            </button>
                          )}
                          <button onClick={() => deleteRule(rule.id)} className="p-1.5 text-slate-400 hover:text-red-500 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20">
                        ...
                        <div className="mt-3 pt-3 border-t border-slate-100 dark:border-slate-700 space-y-3">
                          {isEditing ? (
                            <>
                              ...
                              <div>
                                <label className="text-xs font-medium text-slate-500 dark:text-slate-400 mb-1 block">Explanation</label>
                              ...
                              <div className="flex justify-end gap-2">
                                <button onClick={() => setEditingId(null)} className="px-3 py-1.5 text-sm text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg">Cancel</button>
                            ...
                            <>
                              {rule.explanation && <p className="text-xs text-slate-600 dark:text-slate-300">{rule.explanation}</p>}
                              {rule.formula?.ranges && (
                                <div className="overflow-x-auto">
                                  <table className="text-xs w-full">
                                    <thead><tr className="text-slate-400 dark:text-slate-500"><th className="text-left py-1">Sqft Range</th><th className="text-right py-1">Price</th></tr></thead>
                                    <tbody>
                                      {rule.formula.ranges.map((r: any, i: number) => (
                                        <tr key={i} className="border-t border-slate-50 dark:border-slate-800">
                                          <td className="py-1 text-slate-600 dark:text-slate-300">{r.min.toLocaleString()} – {r.max >= 99999 ? "∞" : r.max.toLocaleString()} sqft</td>
                                          <td className="py-1 text-right font-medium text-slate-900 dark:text-white">${r.price}</td>
                                        </tr>
                                      ))}
                                    </tbody>
                                  </table>
                              ...
                              {typeof rule.formula?.value === "object" && !rule.formula?.ranges && (
                                <div className="flex flex-wrap gap-2">
                                  {Object.entries(rule.formula.value).map(([k, v]: any) => (
                                    <span key={k} className="text-xs px-2 py-1 bg-slate-100 dark:bg-slate-700 rounded-lg text-slate-600 dark:text-slate-300">
                                      <strong className="capitalize">{k.replace("-", " ")}:</strong> {typeof v === "number" ? (v < 10 ? `${v}x` : `$${v}`) : `${v}%`}
                                    </span>
                                  ))}
                                </div>
                              )}
                              <p className="text-xs text-slate-400 dark:text-slate-500">Variables: {rule.inputVariables?.join(", ") || "none"}</p>
                            </>
                          )}
                        </div>
                      )}
...
function CalculatorStep({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Quote Calculator</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Enter a new job and get an instant quote based on your published pricing rules.</p>
      </div>
      ...
      {!hasPublished && (
        <div className="flex items-center gap-3 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800">
          ...
          <div>
            <p className="text-sm font-medium text-amber-700 dark:text-amber-300">Rules not published yet</p>
            <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">Go back to the Analysis step, review your rules, and click "Publish Pricing Rules" to activate the calculator.</p>
          </div>
        </div>
      )}

      {hasPublished && (
        <div className="flex items-center gap-2 p-3 bg-green-50 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800 text-sm text-green-700 dark:text-green-300">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <Card>
          <div className="p-5 space-y-4">
            <h3 className="text-sm font-semibold text-slate-900 dark:text-white">Job Details</h3>
          ...
          {result ? (
            <>
              <Card>
                <div className="p-5 text-center">
                  <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase mb-1">Recommended Quote</p>
                  <p className="text-5xl font-bold text-primary-600 dark:text-primary-400">${result.total.toFixed(0)}</p>
                  <p className="text-xs text-slate-400 dark:text-slate-500 mt-2">{result.rulesApplied} rule{result.rulesApplied !== 1 ? "s" : ""} applied</p>
                </div>
              </Card>
              ...
              <Card>
                <div className="p-4">
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase mb-3">Price Breakdown</p>
                  <div className="space-y-2">
                    {result.breakdown.map((line, i) => (
                      <div key={i} className="flex items-center justify-between text-sm">
                        ...
                        <span className={`font-medium ${line.effect < 0 ? "text-green-600 dark:text-green-400" : line.type === "floor" ? "text-amber-600 dark:text-amber-400" : "text-slate-900 dark:text-white"}`}>
                    ...
                    <div className="border-t border-slate-100 dark:border-slate-700 pt-2 flex items-center justify-between font-semibold">
                      <span className="text-slate-900 dark:text-white">Total</span>
                      <span className="text-primary-600 dark:text-primary-400 text-lg">${result.total.toFixed(2)}</span>
                    </div>
                  </div>
                </div>
              </Card>
            ...
            <Card>
              <div className="py-20 text-center">
                ...
                <p className="text-sm text-slate-400 dark:text-slate-500">Fill in the job details and click Calculate to see your quote.</p>
              </div>
            </Card>
          )}
        </div>
      ...
      <div className="p-6 max-w-4xl mx-auto">
        <div className="min-h-[70vh] flex flex-col items-center justify-center text-center space-y-8">
          ...
          <div className="max-w-xl space-y-3">
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Pricing Logic Engine</h1>
            <p className="text-lg text-slate-500 dark:text-slate-400">Upload past quotes and we'll build your pricing system — so you quote faster, more consistently, and stop leaving money on the table.</p>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-2 px-8 py-3.5 bg-primary-600 text-white rounded-2xl font-semibold text-lg hover:bg-primary-700 transition-colors shadow-lg shadow-primary-200 dark:shadow-none"
          >
            <Brain className="w-5 h-5" />
            Build My Pricing Logic
          </button>

          <p className="text-xs text-slate-400 dark:text-slate-500">Takes about 5 minutes. You can edit everything before publishing.</p>
        </div>
      </div>
    );
  }
