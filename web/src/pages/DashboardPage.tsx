...
function TodayOperations({
 ...
 return (
 <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden mb-6 shadow-sm transition-all duration-200 ease-in-out hover:shadow-md hover:border-slate-300">
...
function StartHereChecklist({
 ...
 <div>
 <h2 className="font-bold text-slate-800 text-sm">Complete setup to unlock full automation</h2>
 <p className="text-xs text-indigo-600 mt-0.5">{completedCount} of {steps.length} steps done · {steps.length - completedCount} remaining</p>
 </div>
 </div>
 ...
 <p className="text-sm font-semibold text-slate-800 flex-1">{nextStep.label}</p>
 <button
...
export default function DashboardPage() {
 ...
 {/* 1b. Trial-expired banner — non-dismissible, shown when free trial is over */}
 {trialExpired && user?.subscriptionTier === "free" && (
   <div className="w-full mb-6 rounded-2xl overflow-hidden border border-amber-300/60 bg-gradient-to-r from-amber-50 via-orange-50 to-amber-50 shadow-sm">
     <div className="px-5 py-4 flex items-center gap-4">
       ...
       <div className="flex-1 min-w-0">
         ...
         <p className="text-xs text-slate-600 mt-0.5 leading-relaxed">
           Upgrade to keep access to your quotes and automations.
         </p>
       </div>
       <button
         onClick={() => startCheckout("growth", "monthly")}
         disabled={checkoutLoading}
         className="shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-white text-xs font-bold transition-colors shadow-sm shadow-amber-500/30"
       >
         {checkoutLoading ? (
