...
const FEATURES = [
  ...
  { icon: "pause",     text: "Pause or resume any lead at any time" },
];

const AUTOPILOT_OFFERING_ID = "autopilot";
...
export function AutopilotUpsellModal({ visible, onClose, tier, embedded = false }: Props) {
  ...
  const handleUpgradeGrowth = async () => {
    ...
    // iOS/Android: use RevenueCat in-app purchase
    if (Platform.OS === "ios" || Platform.OS === "android") {
      ...
      try {
        ...
        const autopilotOffering = offerings.all[AUTOPILOT_OFFERING_ID];
        const pkg = autopilotOffering?.monthly
          ?? autopilotOffering?.availablePackages?.[0]
          ?? null;

        if (!pkg) {
          // Offering not yet configured in RC dashboard — fall back to web
        ...
        // Purchase completed — enable autopilot on the backend
        await apiRequest("POST", "/api/autopilot/settings", { autopilotEnabled: true });
        ...
        Alert.alert(
          "Purchase failed",
          "We could not complete your purchase. Please try again or contact support.",
          [{ text: "OK" }]
        );
      } finally {
        setPurchasing(false);
    ...
    <View style={[
      ...
      <View style={[styles.featureList, { borderColor: theme.border }]}>
        {FEATURES.map((f, i) => (
          <View
            ...
            style={[
              styles.featureRow,
              i < FEATURES.length - 1 && { borderBottomWidth: 1, borderBottomColor: theme.border },
            ]}
          >
      ...
      {isGrowth ? (
        <Pressable
          ...
          style={({ pressed }) => [
            styles.primaryBtn,
            { backgroundColor: theme.primary, opacity: pressed || purchasing ? 0.75 : 1 },
          ]}
        ...
        >
          ...
          <ThemedText style={styles.primaryBtnText}>
            {purchasing ? "Processing…" : "Add Autopilot — $29/mo"}
          </ThemedText>
        </Pressable>
      ) : (
        <Pressable
      ...
      {!embedded && (
        <Pressable onPress={onClose} style={styles.dismissBtn} disabled={purchasing}>
